...
- **AI Agent 动作指引**：
    ...
    - 若项目根目录下存在 `.python-version` 文件，同步将其内容改为 `3.13.13`（对应镜像版本去掉时间戳）。
    ...
    try:
        ...
        if violations:
            ...
            for lineno, full_tag, suffix in violations:
                ...
                clean_suffix = (
                    suffix[version_and_ts_match.end() :]
                    if version_and_ts_match
                    else suffix
                )
...
def main():
    ...
    if total_violations == 0:
        print(
            "[镜像版本检查]: 成功。Dockerfile 和 .woodpecker/ 目录下的 Python 3.13 镜像均已对齐。"
        )
        sys.exit(0)
    else:
        ...
        print(
            "2. 使用 `replace` 工具将不合规的 `python-driver:3.13...` 替换为 `python-driver:{TARGET_VERSION}`。"
        )
        print(
            "   注意：需移除旧的版本号和旧的时间戳，但保留 `-slim` 等后缀。替换后请复查。"
        )
        print(
            "3. 若项目根目录下存在 `.python-version` 文件，同步将其内容修改为 `3.13.13` (即镜像版本去掉时间戳后缀)。"
        )
