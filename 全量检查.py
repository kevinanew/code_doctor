...
## 1. 目标
提供一个统一的入口点，用于自动运行当前目录下所有的代码检查脚本。这简化了执行流程，让用户可以通过一个命令运行所有检查项。
...
- **执行逻辑**：
    - 遍历符合条件的脚本文件。
    - 为每个脚本启动一个子进程，并将目标目录（命令行参数）传递给它们。
    - 捕获并汇总每个检查脚本的输出。
- **结果输出**：打印每个检查工具的运行结果，最后显示汇总统计信息。

## 3. 命令行接口
- **用法**：`python 全量检查.py <target_directory>`
...
def main():
    if len(sys.argv) != 2:
        print("用法: python 全量检查.py <target_directory>")
        sys.exit(1)
