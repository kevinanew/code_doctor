...
- **全量检查**：提供统一的 `全量检查.py` 脚本，可以一次性运行目录下的所有检查工具。
...
可以使用 `全量检查.py` 自动发现并运行当前目录下的所有检查脚本：

```bash
python 全量检查.py /path/to/your/project
```
...
5. **自动化测试**：每一个脚本都必须配备相应的测试脚本，测试脚本命名规范为 `测试_<脚本名>.py`。
