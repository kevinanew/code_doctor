...
*   **避免失效**：如果测试文件移出 `tests/` 但 `conftest.py` 仍留在其中，新的测试将无法识别旧的配置。通过向上移动，可以确保配置覆盖范围扩大，维持测试环境的一致性。
...
def 执行配置归位检查(目标目录, verbose=False):
    ...
    if not 配置文件偏移列表:
        ...
        for 配置项 in 配置文件偏移列表:
            ...
            print(
                f"   - 归位操作：**必须使用 `git mv`** 将其向上移动一层到 '{配置项['建议目标目录']}'。"
            )
            print(
                "     原因：测试文件已移出旧目录，配置文件需上移以维持 fixture 作用域。"
            )
