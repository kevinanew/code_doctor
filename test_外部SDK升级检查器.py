...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "external_sdk_upgrade_checker", script_path
    )
...
class TestExternalSdkUpgradeChecker(unittest.TestCase):
    ...
    def test_resolve_reference_sdk_dir_prefers_flask_then_same_then_sanic(self):
        self.write_file(
            self.reference_sdk_root, "sample_flask/sample_flask.py", "value = 1\n"
        )
        self.write_file(self.reference_sdk_root, "sample/sample.py", "value = 2\n")
        self.write_file(
            self.reference_sdk_root, "sample_sanic/sample_sanic.py", "value = 3\n"
        )

        checker = self.module.ExternalSDKUpgradeChecker(
            Path("."), self.module.REFERENCE_REPO_ROOT, self.reference_sdk_root
        )
    ...
    def test_resolve_reference_sdk_dir_falls_back_to_same_name_then_sanic(self):
        ...
        self.write_file(
            self.reference_sdk_root, "demo_sanic/demo_sanic.py", "value = 3\n"
        )
    ...
    def test_resolve_reference_sdk_dir_maps_room_sdks_to_room_sanic(self):
        self.write_file(
            self.reference_sdk_root, "room_sanic/room_sanic.py", "value = 3\n"
        )
    ...
    def test_resolve_reference_sdk_dir_maps_user_profile_sdks_to_user_profile_flask(
        self,
    ):
        self.write_file(
            self.reference_sdk_root,
            "user_profile_flask/user_profile_flask.py",
            "value = 3\n",
        )
        ...
        for sdk_name in ("user_profile", "user_profile_sanic"):
            with self.subTest(sdk_name=sdk_name):
                ...
                resolved = checker.resolve_reference_sdk_dir(sdk_name)
                self.assertEqual(
                    resolved, self.reference_sdk_root / "user_profile_flask"
                )
    ...
    def test_resolve_reference_repo_root_prefers_home_github_path(self):
        ...
        with (
            patch.object(self.module.Path, "home", return_value=home_root),
            patch.object(
                self.module,
                "REFERENCE_REPO_CANDIDATES",
                (self.workspace / "missing_api_sdk", api_sdk_root),
            ),
        ):
            resolved = (
                self.module.ExternalSDKUpgradeChecker.resolve_reference_repo_root()
            )
    ...
    def test_collect_sdk_statuses_filters_self_and_classifies(self):
        self.write_file(
            self.current_sdk_root, "sample_project/sample_project.py", "self = 1\n"
        )
        self.write_file(
            self.current_sdk_root,
            "need_update/need_update.py",
            self.multi_line_content("current", 12),
        )
        self.write_file(
            self.current_sdk_root,
            "already/already.py",
            self.multi_line_content("same", 3),
        )
        self.write_file(
            self.current_sdk_root,
            "room/room.py",
            self.multi_line_content("room current", 12),
        )
        self.write_file(
            self.current_sdk_root,
            "room_v10/room_v10.py",
            self.multi_line_content("room v10 current", 12),
        )
        ...
        self.write_file(
            self.reference_sdk_root,
            "need_update/need_update.py",
            self.multi_line_content("reference", 12),
        )
        self.write_file(
            self.reference_sdk_root,
            "already/already.py",
        ...
        self.write_file(
            self.reference_sdk_root,
            "sample_project/sample_project.py",
            self.multi_line_content("ref", 3),
        )
        self.write_file(
            self.reference_sdk_root,
            "room_sanic/room_sanic.py",
            self.multi_line_content("room reference", 12),
        )

        with patch.object(
            self.module.ExternalSDKUpgradeChecker,
            "resolve_reference_repo_root",
            return_value=self.reference_root,
        ):
            checker = self.module.ExternalSDKUpgradeChecker(
                self.project_root,
                self.module.REFERENCE_REPO_ROOT,
                self.reference_sdk_root,
            )
        ...
        self.assertEqual(
            [item.sdk_name for item in outdated], ["need_update", "room", "room_v10"]
        )
    ...
    def test_select_upgrade_batch_prefers_non_group_before_room_group(self):
        ...
        self.assertEqual(
            [item.sdk_name for item in remaining], ["room_v10", "user_profile"]
        )
    ...
    def test_select_upgrade_batch_uses_user_profile_group_after_room_sdks_are_done(
        self,
    ):
        outdated = [
            self.module.SDKStatus(
    ...
    def test_build_prompt_uses_group_specific_header(self):
        ...
        unknown = [
            self.module.SDKStatus(
                "sdk_z", Path("/a"), None, None, None, "unknown", "当前项目主文件缺失"
            ),
        ]

        prompt = self.module.PromptLibrary.render_prompt(
            self.project_root, selected, unknown, 4, self.module.SDK_UPGRADE_GROUPS[0]
        )
        ...
        self.assertIn(
            "普通 SDK 的参考仓库选择优先级：先 `_flask`，其次完全同名，最后 `_sanic`",
            prompt,
        )
    ...
    def test_build_prompt_uses_user_profile_group_specific_header(self):
        ...
        prompt = self.module.PromptLibrary.render_prompt(
            self.project_root, selected, [], 1, self.module.SDK_UPGRADE_GROUPS[1]
        )
    ...
    def test_main_blocks_when_dirty_sdk_changes_exist(self):
        ...
        with (
            patch.object(
                self.module.ExternalSDKUpgradeChecker,
                "collect_dirty_sdk_paths",
                return_value=["src/sdk/room/room.py"],
            ),
            patch.object(
                self.module.ExternalSDKUpgradeChecker,
    ...
    def test_main_returns_zero_when_no_upgrade(self):
        ...
        self.write_file(
            self.reference_sdk_root, "other_sdk/other_sdk.py", "value = 1\n"
        )
        self.write_file(self.current_sdk_root, "room/room.py", "value = 10\n")
        self.write_file(
            self.reference_sdk_root, "room_sanic/room_sanic.py", "value = 10\n"
        )
    ...
    def test_main_returns_zero_when_reference_repo_missing(self):
        ...
        self.assertEqual(return_code, 0)
        self.assertIn(
            "未找到 api_sdk 参考项目，跳过非本项目 SDK 升级检查。", buffer.getvalue()
        )
    ...
    def test_main_returns_one_when_outdated_sdk_exists(self):
        ...
        self.write_file(
            self.current_sdk_root, "room/room.py", self.multi_line_content("same", 3)
        )
        self.write_file(
            self.reference_sdk_root,
            "room_sanic/room_sanic.py",
    ...
    def test_count_diff_lines_treats_small_changes_as_no_upgrade(self):
        ...
        self.assertEqual(
            self.module.ExternalSDKUpgradeChecker.diff_lines(
