...
- **参考仓库更新规则**：检查前必须先更新 `api_sdk` 的 `master` 分支，且必须按以下顺序执行：
  ...
  如果更新失败，允许继续使用当前本地参考仓库内容进行后续检查，但需要记录更新失败状态。
...
- **SDK 补齐规则**：必须确保 `sdk_client.py` 与 `test_sdk_client.py` 存在；这两个文件只能放在当前项目 `src/sdk/` 根目录下，供所有 SDK 共用，不允许放进任何 SDK 子目录。若缺失，则从参考项目中复制到当前项目 `src/sdk/` 根目录下。
- **对齐规则**：需要对齐当前项目与参考仓库中对应 SDK 的所有文件。
- **修改边界**：不要修改本项目中与上述 SDK 无关的内容。
- **结构一致性**：更新后保持文件结构与命名一致。
- **room 迁移规则**：`room` / `room_v10` 的最终标准实现是 `room_sanic`，检查器生成的提示词必须明确这一点。
- **profile 迁移规则**：`user_profile` 只属于 `user_profile` 组，不允许混入 `room` 组。
...
| 失败 | git 拉取失败 | 记录更新失败状态，但继续后续流程 | 无 |
...
- 当前项目根目录
- 参考仓库中 SDK 所在目录
- 无法判断的 SDK 名称列表，且说明必须直接写在项目名后面，例如 `- sdk_name：原因`
- 参考仓库 SDK 的选择优先级必须先选 `_flask`，其次选完全同名，最后选 `_sanic`
- 对齐与补齐要求
- 文件结构与命名约束
- 修改边界约束
...
- 当存在无法判断的 SDK 时，必须单独输出这些 SDK 的名称，并在同一行直接写明无法判断原因
- 说明参考仓库 SDK 的选择优先级必须先选 `_flask`，其次选完全同名，最后选 `_sanic`
...
**提示词要求**
- 明确列出 SDK 名称
- 明确说明当前项目根目录与参考仓库的 SDK 目录，且参考仓库的 `sdk_client.py` 与 `test_sdk_client.py` 位于该 SDK 目录下
- 明确说明 `sdk_client.py` 与 `test_sdk_client.py` 必须放在当前项目 `src/sdk/` 根目录下，所有 SDK 共用，不要放进某个 SDK 子目录
- 明确说明参考仓库 SDK 的选择优先级是先 `_flask`，其次完全同名，最后 `_sanic`，脚本会先自行判断
- 明确要求对齐主文件
- 明确要求补齐 `sdk_client.py` 与 `test_sdk_client.py`，如果当前项目缺失则从参考项目复制，并且两个文件只能存在于 `src/sdk/` 根目录
...
- 明确要求无法判断的 SDK 说明必须直接写在项目名后面，例如 `- sdk_name：原因`
- 提示词本身应成为脚本最终输出的唯一内容
