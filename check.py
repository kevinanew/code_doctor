...
- **执行逻辑**：
    ...
    - **快速失败**：如果任何检查脚本运行失败（返回非零退出码），立即停止执行后续脚本并引导 AI Agent 在新分支提交代码并使用 gh pr create 命令创建 PR（**PR 标题和描述必须使用中文，且不要做任何优化**）。
    - 在每个脚本运行结束后，立即检查 Git 状态。
    - **变更中断**：如果发现文件变动，必须立即停止后续检查，并引导 AI Agent 在新分支提交代码并使用 gh pr create 命令创建 PR（**PR 标题和描述必须使用中文，且不要做任何优化**），且不再进行本地检查。
