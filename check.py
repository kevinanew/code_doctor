...
- **执行逻辑**：
    ...
    - **快速失败**：如果任何检查脚本运行失败（返回非零退出码），立即停止执行后续脚本并引导 AI Agent 在新分支提交代码并直接创建 PR（PR 标题和描述使用中文）。
    - 在每个脚本运行结束后，立即检查 Git 状态。
    - **变更中断**：如果发现文件变动，必须立即停止后续检查，并引导 AI Agent 在新分支提交代码并直接创建 PR（PR 标题和描述使用中文），且不再进行本地检查。
    ...
    for script in scripts:
        ...
        try:
            ...
            if is_git_modified():
                ...
                print("2. **提交并创建 PR**：请基于当前修改在新的 Git 分支上提交代码，并直接创建一个 PR（PR 标题和描述请使用中文）。")
