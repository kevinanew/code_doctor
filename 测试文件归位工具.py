...
## 2. 归位规则
*   **归位目标**：
    *   **测试文件**：源代码 `path/to/文件名.py` 对应的测试必须位于 `path/to/test_文件名.py`。
    *   **配置文件 (conftest.py)**：如果 `conftest.py` 位于 `unittests` 或 `tests` 等旧的测试目录下，必须**向上移动一层**（通常移动到项目根目录或源代码同级目录）。
*   **寻找来源**：工具会在**目标目录**（即命令行指定的目录）下全局搜索名为 `test_文件名.py` 和 `conftest.py` 的文件。
*   **归位执行规范**：
...
def 执行归位检查(目标目录, verbose=False):
    ...
    # 1. 扫描目标目录，记录所有已经存在的 test_*.py 和 conftest.py 文件及其当前位置
    全局测试库 = {}
    配置文件偏移列表 = []
    ...
    for 根路径, 目录, 文件名列表 in os.walk(目标目录):
        ...
        for 文件 in 文件名列表:
            ...
            if 文件 == "conftest.py":
                ...
                # 如果 conftest.py 位于排除关键字目录中（如 unittests/conftest.py）
                if any(关键字 in 根路径 for 关键字 in 排除关键字):
                    # 建议向上移动一层
    ...
    if verbose:
        print(f"[*] 已建立全局库，记录了 {len(全局测试库)} 个测试文件和 {len(配置文件偏移列表)} 个位置错误的配置文件。")
    ...
    # 3. 输出报告
    if not 需要归位列表 and not 配置文件偏移列表:
        print(f"[归位检查]: 成功 (目录: {目标目录})")
        print("该目录下所有已发现的测试文件和配置文件均已正确归位。")
        sys.exit(0)
    else:
        print(f"[归位检查]: 发现位置错误的测试或配置文件 (目录: {目标目录})")
        print("\n[AI Agent 动作指引]:")
        ...
        print("4. **无需清理空目录**：无需运行 `git clean`，空目录留在本地无碍。")
        print("5. **内容零改动**：应优先仅移动物理位置。通常不需要修改 `import`，尽量保持文件内容不变。")
