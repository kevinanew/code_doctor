...
*   **归位执行规范**：
    *   **绝对禁止编写新测试**：本项目拒绝为了通过工具检查而“临时凑数”编写的无意义测试。若源代码对应的测试文件不存在或不匹配，**必须且仅能**通过创建“占位文件”来解决。
...
*   **占位文件规范**：
    *   文件名：`test_文件名.py`。
    *   内容：**严禁编写任何 import 或测试逻辑，严禁为了通过检查而编写新的测试用例**。仅允许包含一行注释：`# 为避免归位误报自动生成的占位测试`。
    ...
    if not 需要归位列表:
        ...
        if 需要归位列表:
            ...
            for 归位项 in 需要归位列表:
                ...
                print(
                    f"   3. 占位：如果不匹配（误报），或者该代码文件确实不需要测试，**严禁编写新的测试逻辑**。请在 '{归位项['归位目标']}' 仅创建一个包含一行注释的文件："
                )
